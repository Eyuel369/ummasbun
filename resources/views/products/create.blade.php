<x-app-shell>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold text-slate-900">Add Product</h1>
            <p class="text-sm text-slate-500">Create a new menu item.</p>
        </div>
    </x-slot>
    ...
    <div class="space-y-4">
        ...
        <x-card>
            <form method="POST" action="{{ route('products.store') }}" class="space-y-5">
                ...
                <x-input label="Name" name="name" value="{{ old('name') }}" placeholder="Product name" />
                <x-input-error :messages="$errors->get('name')" />

                <x-input label="Price" name="price" type="number" step="0.01" min="0" value="{{ old('price') }}" placeholder="0.00" />
