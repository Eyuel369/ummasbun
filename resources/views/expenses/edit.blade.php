<x-app-shell>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold text-slate-900">Edit Expense</h1>
            <p class="text-sm text-slate-500">Update category and amount details.</p>
        </div>
    </x-slot>
    ...
    <div class="space-y-4">
        ...
        <x-card>
            <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-5">
                ...
                <x-input label="Date" name="expense_date" type="date" value="{{ old('expense_date', $expense->expense_date->toDateString()) }}" />
                ...
                <x-input label="Amount" name="amount" type="number" step="0.01" min="0.01" value="{{ old('amount', $expense->amount) }}" placeholder="0.00" />
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="description" name="description" rows="3" class="block w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-slate-400 focus:ring-2 focus:ring-slate-200" placeholder="Optional notes">{{ old('description', $expense->description) }}</textarea>
                </div>
